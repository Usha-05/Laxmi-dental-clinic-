    ...
    <section id="assess-yourself" className="w-full py-8 md:py-12 lg:py-16 bg-gradient-to-b from-[#d1fae5] via-[#ecfdf5] to-white relative">
      ...
      <div className="w-full px-2 sm:px-4 lg:px-6 relative z-10">
        <div className="text-center mb-8 md:mb-12">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-foreground mb-4 tracking-tight">Assess Yourself</h2>
        ...
        {!selectedType ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {Object.entries(assessments).map(([key, assessment]) => (
              ...
              >
                <CardHeader>
                  <CardTitle className="text-xl sm:text-2xl font-extrabold text-emerald-800 leading-snug">
                    {assessment.title}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <Button className="w-full bg-primary hover:bg-primary/90 text-base md:text-lg py-3 font-semibold tracking-wide shadow-md">
                    Start Assessment
                  </Button>
                </CardContent>
              </Card>
          ...
          <div className="max-w-3xl mx-auto">
            <Card className="border-emerald-100 shadow-lg">
              <CardHeader>
                <Button variant="ghost" className="mb-4 w-fit text-base md:text-lg font-semibold text-primary hover:text-primary/80" onClick={handleReset}>
                  ← Back to Assessments
                </Button>
                <CardTitle className="text-2xl md:text-3xl font-bold text-primary leading-tight">{currentAssessment?.title}</CardTitle>
                <CardDescription className="text-base md:text-lg mt-3 leading-relaxed text-muted-foreground">
                  {currentAssessment?.description}
                </CardDescription>
                <p className="text-sm md:text-base text-amber-700 mt-6 bg-amber-50/80 border border-amber-200 p-4 rounded-xl">
                ...
                {!showResults ? (
                  <div className="space-y-5">
                    {currentAssessment?.questions.map((question) => (
                      <div key={question.id} className="space-y-3 pb-4 border-b border-border/60 last:border-b-0">
                        <Label className="text-lg md:text-xl font-semibold tracking-tight text-foreground">
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            ...
                            <Label htmlFor={`q${question.id}-yes`} className="text-base md:text-lg cursor-pointer text-muted-foreground">
                              Yes
                            </Label>
                          </div>
                          <div className="flex items-center space-x-3">
                            <RadioGroupItem value="no" id={`q${question.id}-no`} className="size-4 shrink-0 border-2 border-primary/40" />
                            <Label htmlFor={`q${question.id}-no`} className="text-base md:text-lg cursor-pointer text-muted-foreground">
                              No
                            </Label>
                          </div>
                        </RadioGroup>
                    ...
                    <div className="pt-5 space-y-4">
                      <Button
                        ...
                        className="w-full bg-primary hover:bg-primary/90 h-12 text-base md:text-lg font-semibold"
                        size="lg"
                      >
                        Assess
                      </Button>
                      <Button onClick={handleReset} variant="outline" className="w-full bg-transparent text-base md:text-lg font-semibold h-12">
                        Clear
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-primary/10 to-primary/5 p-6 rounded-2xl text-center shadow-inner">
                      <p className="text-base md:text-lg text-muted-foreground mb-3 font-semibold tracking-wide uppercase">Your Assessment Score</p>
                      <div className="text-4xl md:text-5xl font-extrabold text-primary mb-3">{eligibilityPercentage}%</div>
