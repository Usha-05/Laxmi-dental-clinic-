...
import { sendVirtualConsultationEmails } from "@/lib/virtual-consultation"
import { sendWhatsAppMessage } from "@/lib/whatsapp-virtual"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.name || !body.email || !body.phone || !body.preferredDate || !body.concern) {
    ...
    console.log("[v0] Virtual consultation saved:", newConsultation)
    ...
    let emailSent = false
    try {
      await sendVirtualConsultationEmails(newConsultation as any)
      emailSent = true
    ...
    console.error("[v0] Virtual consultation API error:", error)
